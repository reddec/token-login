...
func (q *Queries) CreateDefaultProject(ctx context.Context, user string) (Project, error) {
	row := q.db.QueryRow(ctx, createDefaultProject, user)
	var i Project
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
...
SELECT id, created_at, updated_at, "user", slug, description FROM project WHERE "user" = $1 AND slug = ''
`

func (q *Queries) GetDefaultProject(ctx context.Context, user string) (Project, error) {
	row := q.db.QueryRow(ctx, getDefaultProject, user)
	var i Project
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
